...
async def add_restaurant(ctx, link=None):
    ...
    dup_check = db.query(
        f'select * from channel_restaurant where channel_id="{ctx.message.channel.id}" and restaurant_id={res_id}'
    ).fetchone()
    if dup_check is None:
        db.query(
            f'insert into channel_restaurant(channel_id, restaurant_id) values("{ctx.message.channel.id}", {res_id})'
        )
    else:
...
def check_status(name):
    url = f"https://restaurant-api.wolt.com/v3/venues/slug/{name}"
    r = requests.get(url)
    try:
        return r.json().get("results", {})[0].get("alive", 0)
    except IndexError:
        return 0
...
async def list_restaurants(ctx):
    ...
    rows = db.query(
        f'select restaurant_id from channel_restaurant where channel_id="{ctx.message.channel.id}"'
    ).fetchall()
    for row in rows:
